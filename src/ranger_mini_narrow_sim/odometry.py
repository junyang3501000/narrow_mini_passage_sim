...
class OdometryProgress:
    ...
    def __init__(self, max_step: float):
        self._tracker = DistanceTracker(max_step)
        self.latest_pose: Optional[OdomPose] = None
    ...
    def ingest(
        self, message: Odometry, received_at: float, tracking: bool
    ) -> Optional[DistanceUpdate]:
        ...
        if not all(math.isfinite(value) for value in (pose.x, pose.y, pose.yaw)):
            ...
            return (
                DistanceUpdate(False, self.distance, "non_finite_odom")
                if tracking
                else None
            )
    ...
    def is_fresh(self, now: float, timeout: float) -> bool:
        ...
        return (
            ...
            and 0.0 <= now - self.received_at <= timeout
        )
